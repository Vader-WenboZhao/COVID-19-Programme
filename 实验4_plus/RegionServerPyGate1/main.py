...
'''实验记录路径2'''
dataFilePCPart = 'dataWithPCPart.txt'
        ...
        try:
            ...
            if len(localTraces) > 0:
                try:
                    ...
                    '''记录发送trace数据'''
                    record(['send_trace', ''], dataFilePCPart)
                    ...
                    while (not ACKFromPC) and count<=3:
                        ...
                        '''记录重复发送trace数据'''
                        record(['REsend_trace', ''], dataFilePCPart)
...
def recvFromPCPart():
    ...
    while True:
        try:
            ...
            '''记录接收到ACK数据'''
            record(['recv_ACK', ''], dataFilePCPart)
        ...
        try:
            ...
            if (len(recv_pkg) > 2):
                ...
                if 'timeAsk' in msg.keys() and msg['timeAsk']==True:
                    ...
                    '''记录接收到时间矫正请求'''
                    # record(['recv_timestamp_request', str(msg)], dataFileFixedDevice)
                ...
                # 接收到trace数据
                    ...
                    '''记录接收到trace'''
                    # record(['recv_trace', str(msg)], dataFileFixedDevice)
                    ...
                    '''记录发送ACK'''
                    # record(['send_ACK', str(ack_pkg)], dataFileFixedDevice)
