...
# 创建交trace请求
def new_traces(blockchain, connectedAddrList, traces):
    for trace in traces:
        newTrace = Trace(trace['pseudonym'], trace['location'], trace['timestamp'], trace['signature'])
        if newTrace.verify(PUB_KEY_PATH):
            index = blockchain.add_trace(newTrace)
        else:
            print("Invalid trace!")
            continue
